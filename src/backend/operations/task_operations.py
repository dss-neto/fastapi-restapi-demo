...
def get_task_data(task_id: int, owner_user_id: int):
...
def operation_create_task(task, decoded_token_data: dict):
    with Session(engine) as session:
        task_info = Task(
            title = task.title,
...
def operation_get_single_task(task_id: int, decoded_token_data: dict):
    ...
    task_data = get_task_data(task_id, decoded_token_data["sub"])
